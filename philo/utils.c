...
/*   Updated: 2024/06/09 16:14:32 by mal-mora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_msg(t_philo *philo, int state)
{
	...
	time = gettime() - get_long(&philo->table->m_s_dinner,
			&philo->table->s_dinner);
	if (philo->isfull)
		return ;
...
void	ft_usleep(t_table *table, long time)
{
	...
	{
		if (get_bool(&table->m_enddinner, &table->end_dinner))
			return ;
		usleep(500);
	}
}
