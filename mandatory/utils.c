...
/*   Updated: 2024/05/01 13:31:05 by mal-mora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    long time = gettime() - get_long(&philo->table->m_start_dinner, &philo->table->start_dinner) ;
    if (philo->isfull)
        return ;
...
int is_philos_full(t_table *table)
{
    ...
    {
        if (!get_bool(&table->philos_list[i].m_isfull, &table->philos_list[i].isfull))
            return 0;
        i++;    
    }
    return 1;
